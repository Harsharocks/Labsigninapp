...
// Include the checking.php logic here
...
if (isset($_POST['rNumber'])) {
    ...
    // Query to check if the R-number exists in the users table
    $user_sql = "SELECT * FROM users WHERE r_number = '$rNumber'";

    $user_result = $conn->query($user_sql);

    if ($user_result->num_rows > 0) {
        // R-number exists in the users table
        $row = $user_result->fetch_assoc();
        $rNumber = $row['r_number'];
        $name = $row['first_name'];

        // Query to check if the R-number exists in the checkins table and checkout_time is '00:00:00'
        ...
        // R-number does not exist in the users table
